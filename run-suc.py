...
def run(func_args):
    ...
    if func_args.mode == 'train':
        ...
        print(hyper)
        hyper['device'] = 'cuda' if torch.cuda.is_available() else 'cpu'  # 수정된 부분
        ...
        if func_args.market == 'DJIA':
            ...
            print("Loaded Stocks Data Shape:", stocks_data.shape)
            print("Loaded Rate of Return Data Shape:", rate_of_return.shape)
            assert stocks_data.shape[:-1] == rate_of_return.shape, 'file size error'
            A = torch.from_numpy(np.load(matrix_path)).float().to(hyper['device'])  # 수정된 부분
            test_idx = 7328
            allow_short = True
        ...
        func_args.num_assets = stocks_data.shape[0]  # 수정된 부분
        ...
        supports = [A]
        actor = RLActor(supports, func_args).to(hyper['device'])  # 수정된 부분
...
if __name__ == '__main__':
    ...
    parser.add_argument('--window_len', type=int, default=20)  # window_len 인자 추가
    parser.add_argument('--G', type=int)
    parser.add_argument('--batch_size', type=int, default=32)  # batch_size 인자 추가
    parser.add_argument('--seed', type=int, default=-1)
    parser.add_argument('--lr', type=float, default=1e-6)  # lr 인자 추가
    ...
    parser.add_argument('--relation_file', type=str, default='industry_classification.npy')  # 기본값 설정
    parser.add_argument('--addaptiveadj', dest='addaptiveadj', action='store_true')  # addaptiveadj 인자 추가
    parser.add_argument('--market', type=str, default='DJIA')  # market 인자 추가
    parser.add_argument('--mode', type=str, default='train')  # mode 인자 추가
    parser.add_argument('--in_features', type=int, nargs='+', default=[6, 4])  # in_features 인자 추가
    parser.add_argument('--trade_len', type=int, default=21)  # trade_len 인자 추가
    parser.add_argument('--max_steps', type=int, default=12)  # max_steps 인자 추가
    parser.add_argument('--norm_type', type=str, default='div-last')  # norm_type 인자 추가
    parser.add_argument('--hidden_dim', type=int, default=128)  # hidden_dim 인자 추가
    parser.add_argument('--dropout', type=float, default=0.5)  # dropout 인자 추가
    parser.add_argument('--kernel_size', type=int, default=2)  # kernel_size 인자 추가
    parser.add_argument('--num_blocks', type=int, default=4)  # num_blocks 인자 추가
    parser.add_argument('--weight_decay', type=float, default=0.01)  # weight_decay 인자 추가
    parser.add_argument('--epochs', type=int, default=1500)  # epochs 인자 추가
    args = parser.parse_args()
    args.device = 'cuda' if torch.cuda.is_available() else 'cpu'  # 수정된 부분
    run(args)